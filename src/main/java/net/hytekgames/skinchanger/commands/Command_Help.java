...
public class Command_Help implements CommandExecutor {
	...
	public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
		...
		if(!subcommand.isPresent()) {
			...
			p.sendMessage(Text.builder(SkinChanger.prefix+" \u00a7aActual version: "+Sponge.getPluginManager().getPlugin("skinchanger").get().getVersion().get()).build());
			return CommandResult.success();
		}else if(subcommand.get().equals("github")){
			try {
				p.sendMessage(Text.builder(SkinChanger.prefix+" \u00a7aGithub link: https://github.com/TeKGameR950/SkinChanger").onClick(TextActions.openUrl(new URL("https://github.com/TeKGameR950/SkinChanger"))).build());
